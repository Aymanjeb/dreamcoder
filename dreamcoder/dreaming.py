...
def helmholtzEnumeration(g, request, inputs, timeout, _=None,
    ...
    message = json.dumps(message, default=lambda obj: obj.toJson())
...
def backgroundHelmholtzEnumeration(tasks, g, timeout, _=None,
    ...
    #print(tasks[0].examples[0])
    x,y = tasks[0].examples[0]
    print(len(x))
    inputs = {r:[] for r in requests}
    #for r in requests:
    #    for t in tasks:
    #        if t.request == r:
    #            inputs[r].append(list( {tuplify(example[0]) for example in t.examples}))
    inputs = {
        r: [tuplify(xs)
